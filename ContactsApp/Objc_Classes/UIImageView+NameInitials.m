...
#import "UIImageView+NameInitials.h"

// This multiplier sets the font size based on the view bounds
static const CGFloat kFontResizingProportion = 0.42f;

@implementation UIImageView (NameInitials)



- (UIImage *)getImageWithString:(NSString *)string color:(UIColor *)color circular:(BOOL)isCircular {
   return  [self getImageWithString:string color:color circular:isCircular textAttributes:nil];
}

- (UIImage *)getImageWithString:(NSString *)string color:(UIColor *)color circular:(BOOL)isCircular textAttributes:(NSDictionary *)textAttributes {
    if (!textAttributes) {
        textAttributes = @{
    ...
    return [self imageSnapshotFromText:[displayString uppercaseString] backgroundColor:backgroundColor circular:isCircular textAttributes:textAttributes];
}
